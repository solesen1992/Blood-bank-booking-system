...
namespace API.DatabaseLayer
{
    ...
    public class AppointmentAccess : IAppointmentAccess
    {
        ...
        //metode brugte ikke dapper, I stedet bliver SqlCommand og SqlDataReader anvendt direkte til at hente data fra databasen.
        ...
        public Appointment GetAppointmentById(int id)
        {
            ...
            try
            {
                using (var connection = new SqlConnection(ConnectionString))
                {
                    ...
                    // Execute the query and fetch the appointment
                    appointment = connection.QuerySingleOrDefault<Appointment>(query, new { AppointmentId = id });
                }
            }
            catch (SqlException sqlEx)
            {
        ...
        //gamle metode er ikke ordentligt sat op som en transaktion
        public bool DeleteAppointment(Appointment appointment)
        {
            if (appointment == null)
            {
            ...
            using (var connection = new SqlConnection(ConnectionString))
            {
                ...
                using (var transaction = connection.BeginTransaction())
                {
                    try
                    {
                        ...
                        // Execute the query and get the number of affected rows within the transaction
                        int affectedRows = connection.Execute(query, new { AppointmentId = appointment.AppointmentId }, transaction);
                        ...
                        {
                            // Rollback the transaction if no rows were affected (deletion failed)
                            transaction.Rollback();
                        }
                    }
                    catch (SqlException sqlEx)
        ...
        //metoden har ikke brugt Dapper, men SqlCommand og SqlDataReader
        ...
        //Gamle metode har ikke brugt Dapper. Dapper Query erstatter SqlCommand og SqlDataReader med Dappers Query<Appointment>,
        //som automatisk mapper resultatet af SQL-forespørgslen til en liste af Appointment-objekter.
        /*public List<Appointment> GetAppointmentsByDonorId(int donorId)
        {
            // Validate the donorId input to ensure it's a positive integer.
            if (donorId <= 0)
            ...
            try
            {
                ...
                using (var connection = new SqlConnection(ConnectionString))
                {
                 ...
                 SELECT 
                         appointmentId, startTime, endTime, FK_donorId
                     FROM Appointment
                     WHERE FK_donorId = @DonorId";

                    // Execute the query with Dapper to get the appointments and map them to Appointment objects
                    appointments = connection.Query<Appointment>(query, new { DonorId = donorId }).ToList();
                }
            }
            catch (SqlException sqlEx)
            {
